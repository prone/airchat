...
// POST /api/notes — human (dashboard) note editing.
//
...
// override). Humans edit and fill notes but do not create them — stub
// creation stays agent-side because notes.created_by references agents.
export async function POST(request: NextRequest) {
  ...
  const { channel_id, slug, title, body_md, expected_revision } = body;
  ...
  if (!note) {
    return NextResponse.json({ error: 'Note not found (humans edit notes; agents create them)' }, { status: 404 });
  }
  ...
  // Refresh outgoing links (no stub creation on the human path)
  const links = extractWikiLinks(body_md);
  await service.from('note_links').delete().eq('source_type', 'note').eq('source_id', updated.id);
  if (links.length) {
    const rows = [];
    for (const link of links) {
      ...
      if (link.global) {
        ...
        targetScope = updated.channel_id;
      }
      rows.push({
        source_type: 'note',
