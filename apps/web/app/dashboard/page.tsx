...
export default function DashboardPage() {
  ...
  const detectedLinks = useMemo(
    () => extractGithubLinks(messages.map((m) => m.content).join('\n')),
    [messages],
  );
  // Heading repo: explicit metadata repo, else the most-referenced detected one
  const headingRepo = channelRepo || detectedLinks.repos[0] || null;
  ...
  // Channel metadata (github repo) for the heading
  useEffect(() => {
    if (view?.type !== 'channel') { setChannelRepo(null); return; }
    supabase.from('channels').select('metadata').eq('id', view.channel.id).single().then(({ data }) => {
      const repo = (data?.metadata as { github_repo?: string } | null)?.github_repo;
      setChannelRepo(typeof repo === 'string' && repo ? repo : null);
    });
  }, [view, supabase]);
      ...
      <div className="main-panel">
        ...
        <div className="channel-header" style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, flexWrap: 'wrap' }}>
              <h3>{viewTitle}</h3>
              {view?.type === 'channel' && headingRepo && (
                <a
                  href={`https://github.com/${headingRepo}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-xs"
                  style={{ display: 'inline-flex', alignItems: 'center', gap: 4, color: 'var(--accent, #3987e5)' }}
                  title={channelRepo ? 'Linked repo' : 'Repo referenced in this channel'}
                >
                  <svg width="12" height="12" viewBox="0 0 16 16" fill="currentColor" style={{ position: 'relative', top: 1 }}><path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.01 8.01 0 0016 8c0-4.42-3.58-8-8-8z"/></svg>
                  {headingRepo}
                </a>
              )}
            </div>
                  ...
                  {channelAgents.map((a) => {
                    ...
                    return (
                      <div
                        key={a.id}
                        className="agent-row"
                        style={{ display: 'flex', alignItems: 'center', gap: 8, height: 34, padding: '0 8px', borderRadius: 6 }}
                      >
                        ...
                        <button
                          ...
                          style={{ flexShrink: 0, background: 'none', border: 'none', cursor: 'pointer', color: 'var(--accent, #3987e5)', fontWeight: 600 }}
                        >
                          DM
                        </button>
                      </div>
        ...
        {view?.type === 'channel' && (
          <ChannelSummaryPanel
            ...
            isAdmin={isAdmin}
            githubRepo={channelRepo}
            detectedRepos={detectedLinks.repos}
            detectedTasks={detectedLinks.tasks}
            onRepoSaved={setChannelRepo}
          />
        )}
