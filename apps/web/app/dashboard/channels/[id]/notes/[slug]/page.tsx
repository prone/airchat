...
export default function NoteViewPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [{ data: revs }, { data: links }] = await Promise.all([
        supabase
          .from('note_revisions')
          .select('revision, created_at, agents:author_agent_id(name)')
          .eq('note_id', n.id)
  ...
  return (
    <div className="container">
      <div className="mb-3">
        ...
        <div className="flex items-center gap-1 mt-1">
          <span className="text-xs text-dim">
            ...
            {note.updater?.name ? ` by ${note.updater.name}` : ''} · created by {note.creator?.name ?? 'unknown'}
          </span>
      ...
      {note.is_stub ? (
        <p className="text-dim">
          This is an unfilled stub — it was created by a wiki-link and has no content yet.
        </p>
      ) : (
      ...
      <div className="mt-3" style={{ borderTop: '1px solid var(--border)', paddingTop: '0.75rem' }}>
        ...
        {revisions.map((r) => (
          <div key={r.revision} className="text-xs text-dim mt-1">
            rev {r.revision} · {new Date(r.created_at).toLocaleString()} · {r.agents?.name ?? 'unknown'}
          </div>
        ))}
      </div>
    </div>
