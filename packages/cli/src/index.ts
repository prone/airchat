 ...
 * the `create-airchat` package can fold these commands into its own `airchat`
 * bin. `doctor` runs without config; every other command needs `~/.airchat`.
 */
export async function run(argv: string[] = process.argv): Promise<void> {
  ...
  // doctor works without config; only load config for the other commands
  if (argv[2] === 'doctor') {
